...
object IOExercises {
  ...
  /**
    ...
    */
  def helloWorld(logger: String => Unit): IO[Unit] = IO(logger("hello world")) //IO.apply alias for IO.delay, same type signature
  ...
  /**
    ...
    */
  def alwaysFailingTask(): IO[Unit] = IO.raiseError(new Exception) //hint: need a signature that takes a throwable, as Exception extends throwable
  ...
  def logMessageOrFailIfEmpty(msg: String, logger: String => Unit): IO[Unit] = {
    if (msg.isEmpty) IO.raiseError(AppException("Log must not be empty")) //why do we not use the logger here?
    else IO(logger(msg))
  }
  ...
  def getCurrentTempInF(getCurrentTemp: IO[Celsius]): IO[Fahrenheit] = {
    getCurrentTemp.map(celsius => cToF(celsius)) // 1. think about available inputs and match the types
//    2. if cToF was an API we were calling, what might we have to do to only unit test our function
  }
...
//    getCurrentTemp.map(converter) => nested IO
...
//    attempt is on IO's class
  ...
  def mkUsernameThenPrint(username: String, logger: String => Unit): IO[Unit] = {
    mkUsername(username) match {
      case Left(e) => IO.raiseError(e) //what happens if we do IO(e.value)? why do we not want that?
      case Right(str) => IO(logger(str.value))
    }
  }
...
//  }
//  IOExercises.explain(str => println(str)) => is this right for running it?
  def explain(logger: String => Unit): IO[Unit] =
    for {
  ...
  def execute[A](io: IO[A]): A = {
    io.unsafeRunSync() // talk about when unsaferunsync should be used, i.e. in tests or in Main
  }
